...
public class VisualThreadMonitor {
    ...
    private final JScrollPane scrollPane = new JScrollPane(tree, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
                ...
                if(!stackTraces.contains(ste)) {
                    ...
                    for(StackTraceElement e : iste) {
                        if(first && node == top) {
                            ...
                            final DefaultMutableTreeNode n = new DefaultMutableTreeNode(e.toString() + " | " + LocalDateTime.ofInstant(Instant.now(), ZoneId.systemDefault()).format(DateTimeFormatter.ofPattern("HH:mm:ss:SSSS")));
                            node.add(n);
                            node = n;
                            /*
                            DefaultMutableTreeNode n = getChildNode(node, e.toString());
                            if(n == null) {
                                n = new DefaultMutableTreeNode(e.toString());
                                node.add(n);
                            } else if(n == top) {
                                top.setUserObject(e.toString());
                            }
                            */
                        }
                    }
    ...
    private void updateTitle() {
        frame.setTitle(String.format("%s - %s - %s", this.getClass().getSimpleName(), thread.getName(), (thread.isAlive() ? "Alive" : "Dead")));
    }
