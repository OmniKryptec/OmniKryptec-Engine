...
import omnikryptec.terrain.TerrainGenerator;
		...
		try {
			...
			OmniKryptecEngine.instance().addAndSetScene("Test-Scene", new Scene((Camera) new Camera() {
				...
				public void doLogic() {
					...
					InputUtil.doThirdPersonController(this, this,
	...
	private static final void setupTerrains(TerrainTexturePack texturePack, SimpleTexture blendMap, int count) {
		...
		for (int x = 0; x < count; x++) {
			for (int y = 0; y < count; y++) {
				...
				final Terrain terrain = new Terrain(x * heightMap.getWidth(), y * heightMap.getWidth(), modelData,
