...
public class Terrain extends Entity {
    ...
    public Terrain(final float worldX, final float worldZ, ModelData model, TerrainTexturePack texturePack, Texture blendMap) {
        this(worldX, worldZ, new Model(model), texturePack, blendMap);
    }

    public Terrain(final float worldX, final float worldZ, Model model, TerrainTexturePack texturePack, Texture blendMap) {
    ...
    public static final ModelData generateTerrain(final float worldx, final float worldz, final TerrainGenerator generator, final float size, final int vertex_count) {
        ...
        for(int i = 0; i < vertex_count; i++) {
            for(int j = 0; j < vertex_count; j++) {
                final Vertex vertex = new Vertex(vertices.size(), new Vector3f((float) (j / ((float) vertex_count - 1) * size), (float) generator.getHeight(worldx + j, worldz + i), (float) (i / ((float) vertex_count - 1) * size)));
                vertices.add(vertex);
                normal = generator.generateNormal(worldx + j, worldz + i);
                vertex.setNormalIndex(normals.size());
                normals.add(normal);
                texture = new Vector2f((float) (j / ((float) vertex_count - 1)), (float) (i / ((float) vertex_count - 1)));
        ...
        final float furthest = convertDataToArrays(vertices, textures, normals, verticesArray, texturesArray, normalsArray, tangentsArray);
