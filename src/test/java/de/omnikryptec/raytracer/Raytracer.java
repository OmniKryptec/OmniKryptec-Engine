...
public class Raytracer extends Omnikryptec implements ViewRenderer, IUpdatable {
    ...
    private void initShader() {
        ...
        this.dataHelper = new SSBOHelper(1, MAX_SIZE_CUBED);
        this.speedHelper = new SSBOHelper(2, MAX_SIZE_CUBED);
        this.redHelper = new SSBOHelper(3, MAX_SIZE_CUBED);
        this.greenHelper = new SSBOHelper(4, MAX_SIZE_CUBED);
        this.blueHelper = new SSBOHelper(5, MAX_SIZE_CUBED);
        resetSizes();
        updateMode(0);
    }
    ...
    private void updateSizes() {
        ...
        this.size.loadInt(this.currentSize);
    ...
    private void updateMode(int newMode) {
        for (int x = 0; x < this.currentSize; x++) {
            for (int y = 0; y < this.currentSize; y++) {
                for (int z = 0; z < this.currentSize; z++) {
                    float dataV = 0;
                    float speedV = 300000;
                    Color color = new Color();
                    if (newMode == 0) {
                        if (y == 0 || y == this.currentSize - 1) {
                            dataV = 1;
                            if (y == 0) {
                                color.set(0, 1, 0.5f);
                            } else {
                        ...
                        if (y == 0 && x == this.currentSize / 2 && z == this.currentSize / 2) {
                            color.setAll(1);
                        }
                        if (y <= 8 && y != 0) {
                            speedV = 235000;
                        ...
                        if (y > this.currentSize / 3 && y < 2 * this.currentSize / 3f && x > this.currentSize / 3
                                && x < 2 * this.currentSize / 3f && z > this.currentSize / 3
                                && z < 2 * this.currentSize / 3f) {
                            dataV = 0;
                            speedV = 235000;
                            //color.set(0, 0.3f, 0);
                        }
                    ...
                    this.dataHelper.array()[x + y * this.currentSize + z * this.currentSize * this.currentSize] = dataV;
                    this.speedHelper.array()[x + y * this.currentSize
                            + z * this.currentSize * this.currentSize] = speedV;
                    this.redHelper.array()[x + y * this.currentSize + z * this.currentSize * this.currentSize] = color
                            .getR();
                    this.greenHelper.array()[x + y * this.currentSize + z * this.currentSize * this.currentSize] = color
                            .getG();
                    this.blueHelper.array()[x + y * this.currentSize + z * this.currentSize * this.currentSize] = color
                            .getB();
                }
            }
        }
        int currSizeCubed = this.currentSize * this.currentSize * this.currentSize;
        this.dataHelper.push(currSizeCubed);
        this.speedHelper.push(currSizeCubed);
        this.redHelper.push(currSizeCubed);
        this.greenHelper.push(currSizeCubed);
    ...
    public void init(ViewManager vm, RenderAPI api) {
        this.image = (GLFrameBuffer) api.createFrameBuffer(
                (int) (vm.getMainView().getTargetFbo().getWidth() * this.currentResMult),
