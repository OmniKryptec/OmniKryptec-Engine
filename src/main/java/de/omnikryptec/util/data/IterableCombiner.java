...
public class IterableCombiner<T> implements Iterable<T> {
    ...
    private static class CombinedIterator<E> implements Iterator<E> {
        ...
        public boolean hasNext() {
            if (itIndex < this.iterators.length) {
                while (this.iterators[this.itIndex] == null || !this.iterators[this.itIndex].hasNext()) {
                    this.itIndex++;
                    if (this.itIndex >= this.iterators.length) {
                        return false;
                    }
                }
                return this.iterators[this.itIndex].hasNext();
            }
            return false;
        }
