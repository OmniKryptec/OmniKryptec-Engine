...
import de.omnikryptec.event.eventV2.engineevents.CommandEvent;
import de.omnikryptec.util.exceptions.UnsupportedCharacterException;
...
public class Command {
    ...
    private final void preRun(String arguments) {
        if (!arguments.isEmpty() && !usesArguments) {
            Logger.log("Command uses no arguments", LogLevel.ERROR);
            return;
        }
        run(arguments);
    }
