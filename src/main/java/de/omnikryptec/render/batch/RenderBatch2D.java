...
public class RenderBatch2D implements Batch2D {
    ...
    private void draw(Texture texture, Matrix3fc transform, float width, float height, boolean flipU, boolean flipV,
            float u0, float v0, float u1, float v1) {
        checkRendering();
        vertexManager.prepareNext(texture, 6 * FLOATS_PER_VERTEX);
        Vector3f botleft = new Vector3f(0);
        Vector3f botright = new Vector3f(width, 0, 0);
        Vector3f topleft = new Vector3f(0, height, 1);
        Vector3f topright = new Vector3f(width, height, 1);
        botleft = transform.transform(botleft);
        botright = transform.transform(botright);
