...
public class GeometryLoader {
	...
	return new MeshData(verticesArray, texturesArray, normalsArray, tangentsArray, indicesArray, jointIdsArray,
		weightsArray);
    }
	...
	final String positionsId = XMLUtil.getChild(XMLUtil.getChild(meshData, "vertices"), "input")
		.getAttributeValue("source").substring(1);
	final Element positionsData = XMLUtil
		.getChild(XMLUtil.getChildWithAttribute(meshData, "source", "id", positionsId), "float_array");
	...
	for (int i = 0; i < count / 3; i++) {
	    ...
	    vertices.add(new AnimatedVertex(vertices.size(), new Vector3f(position.x, position.y, position.z),
		    vertexWeights.get(vertices.size())));
	}
    }

    private void readNormals() {
	final String normalsId = XMLUtil
		.getChildWithAttribute(XMLUtil.getChild(meshData, "polylist"), "input", "semantic", "NORMAL")
		.getAttributeValue("source").substring(1);
	final Element normalsData = XMLUtil.getChild(XMLUtil.getChildWithAttribute(meshData, "source", "id", normalsId),
		"float_array");
	...
	final String texCoordsId = XMLUtil
		.getChildWithAttribute(XMLUtil.getChild(meshData, "polylist"), "input", "semantic", "TEXCOORD")
		.getAttributeValue("source").substring(1);
	final Element texCoordsData = XMLUtil
		.getChild(XMLUtil.getChildWithAttribute(meshData, "source", "id", texCoordsId), "float_array");
    ...
    private static void calculateTangents(AnimatedVertex v0, AnimatedVertex v1, AnimatedVertex v2,
