...
public class Client extends AdvancedSocket implements InputListenerManager, Serializable {
    ...
    private final AdvancedThreadFactory advancedThreadFactoryProcessor = new AdvancedThreadFactory(
	    "Client-" + Integer.toHexString(hashCode()) + "-Thread-%d");
    /**
	...
	if (object instanceof InputEvent) {
	    ...
	    fireInputEvent(new InputEvent(timestamp, InputType.RAW_MESSAGE_RECEIVED, object),
    ...
    /**
     * Waits for an answer with the given ID (waits about 10 seconds, then returning
     * null)
     * 
     * @param id ID
	...
	while (((event = waitingAnswers.get(id)) == null) && ((maxWaitingDuration == null)
		|| (Duration.between(instantStarted, Instant.now()).compareTo(maxWaitingDuration) < 0))) {
	    try {
		Thread.sleep(1);
	    } catch (Exception ex) {
	    }
     ...
     * (waits about 10 seconds, then returning null)
     * 
     * @param event InputEvent
     * @return Answer or null
     */
	...
	while (((answer = waitingAnswers.get(id)) == null) && ((maxWaitingDuration == null)
		|| (Duration.between(instantStarted, Instant.now()).compareTo(maxWaitingDuration) < 0))) {
	    try {
		Thread.sleep(1);
	    } catch (Exception ex) {
