...
public class KeySettings implements XMLable {
     ...
     */
    public static final KeySettings STANDARDKEYSETTINGS = new KeySettings(new Key[] {
	    new Key("mouseButtonLeft", 0, false), new Key("mouseButtonRight", 1, false),
	    new Key("mouseButtonMiddle", 2, false), new Key("moveForward", GLFW.GLFW_KEY_W, true),
	    new Key("moveBackward", GLFW.GLFW_KEY_S, true), new Key("moveRight", GLFW.GLFW_KEY_D, true),
	    new Key("moveLeft", GLFW.GLFW_KEY_A, true), new Key("moveUp", GLFW.GLFW_KEY_SPACE, true),
	    new Key("moveDown", GLFW.GLFW_KEY_LEFT_SHIFT, true), new Key("turnYawRight", GLFW.GLFW_KEY_RIGHT, true),
	    new Key("turnYawLeft", GLFW.GLFW_KEY_LEFT, true), new Key("turnPitchUp", GLFW.GLFW_KEY_UP, true),
	    new Key("turnPitchDown", GLFW.GLFW_KEY_DOWN, true), new Key("turnRollRight", GLFW.GLFW_KEY_E, true),
	    new Key("turnRollLeft", GLFW.GLFW_KEY_Q, true) });
    ...
    /**
     * Returns if a IKey given by the name is pressed and no other IKey with these
     * keys is pressed
     *
     * @param name String Name of the IKey
     * @return <tt>true</tt> if the IKey is pressed and the only one pressed with
     *         its keys
     */
	...
	if (ikey == null || ikey == Key.DEFAULT_NULL_KEY) {
		...
		for (IKey ikey_ : ikeys) {
			...
			if (keyGroup_.isPressed() && keyGroup_.getKeys().size() > temp.size()
				&& !keyGroup_.getKeys().equals(temp)) {
			    return false;
			}
		    }
		}
    ...
    /**
     * Returns if a IKey given by the name is pressed for a specified time and no
     * other IKey with these keys is pressed
     *
     * @param name    String Name of the IKey
     * @param minTime Float Minimum pressing time
     * @param maxTime Float Maximum pressing time
     * @return <tt>true</tt> if the IKey is pressed for the specified time and the
     *         only one pressed with its keys
     */
     ...
     * @return KeySettings A reference to this KeySettings
     */
    public final KeySettings updateKeys(double currentTime, int key, boolean isKeyboardKey) {
     ...
     * @return KeySettings A reference to this KeySettings
     */
    public final KeySettings updateKeys(ArrayList<IKey> ikeys, double currentTime, int key, boolean isKeyboardKey) {
	for (IKey ikey : ikeys) {
