...
import de.omnikryptec.old.swing.ChartData;
import de.omnikryptec.old.swing.PieChartGenerator;
...
public class LiveProfiler {
    ...
    private final ChartData[] chartDatas = new ChartData[] { new ChartData(Profiler.DISPLAY_IDLE_TIME, 0),
	    new ChartData(Profiler.DISPLAY_UPDATE_TIME, 0), new ChartData(Profiler.SCENE_RENDER_TIME_3D, 0),
	    new ChartData(Profiler.SCENE_LOGIC_TIME_3D, 0), new ChartData(Profiler.SCENE_RENDER_TIME_2D, 0),
	    new ChartData(Profiler.SCENE_LOGIC_TIME_2D, 0), new ChartData(Profiler.PARTICLE_RENDERER, 0),
	    new ChartData(Profiler.PARTICLE_UPDATER, 0), new ChartData(Profiler.POSTPROCESSOR, 0),
	...
	if (omc != null) {
	    // TOD- @Panzer1119 xD
	    vert_info.setText("Frames: " + OmniKryptecEngine.instance().getDisplayManager().getFramecount()
		    + " Particles updated: " + ParticleMaster.instance().getUpdatedParticlesCount()
		    + " Particles rendered: " + ParticleMaster.instance().getRenderedParticlesCount());
	} else {
	...
	frame.setTitle(String.format("LiveProfiler - %s: %f ms - Max %s: %f ms", Profiler.OVERALL_FRAME_TIME,
		(Profiler.currentTimeByName(Profiler.OVERALL_FRAME_TIME)), Profiler.OVERALL_FRAME_TIME, max));
	return updateImage();
    }
