...
public class SerializationUtil {
	...
	return matrix.m00() + MATRIX_SPLITTER + matrix.m01() + MATRIX_SPLITTER + matrix.m02() + MATRIX_SPLITTER
		+ matrix.m03() + MATRIX_SPLITTER + matrix.m10() + MATRIX_SPLITTER + matrix.m11() + MATRIX_SPLITTER
		+ matrix.m12() + MATRIX_SPLITTER + matrix.m13() + MATRIX_SPLITTER + matrix.m20() + MATRIX_SPLITTER
		+ matrix.m21() + MATRIX_SPLITTER + matrix.m22() + MATRIX_SPLITTER + matrix.m23() + MATRIX_SPLITTER
		+ matrix.m30() + MATRIX_SPLITTER + matrix.m31() + MATRIX_SPLITTER + matrix.m32() + MATRIX_SPLITTER
		+ matrix.m33();
    }
	...
	return String.format(Locale.US, "[%f%s%f%s%f%s%f]", vector.x, VECTOR_SPLITTER, vector.y, VECTOR_SPLITTER,
		vector.z, VECTOR_SPLITTER, vector.w);
    }
	...
	temp = temp.substring(1, temp.length() - 1).replaceAll(" ", "");
	final String[] split = temp.split(VECTOR_SPLITTER);
	return new Vector4f(Float.parseFloat(split[0]), Float.parseFloat(split[1]), Float.parseFloat(split[2]),
	...
	return String.format(Locale.US, "[%f%s%f%s%f%s%f]", rotation.x, VECTOR_SPLITTER, rotation.y, VECTOR_SPLITTER,
		rotation.z, VECTOR_SPLITTER, rotation.w);
    }
    ...
    public static final HashMap<Class<?>, ArrayList<GameObject>> gameObjectsToClassesGameObjects(
