...
public class InputManager {
	...
	final Vector2f normalizedDevicePosition = new Vector2f(
		(2.0F * mouseHandler.position.x) / Display.getWidth() - 1,
		(2.0F * mouseHandler.position.y) / Display.getHeight() - 1);
     ...
     * @return
     */
    public static final GameObject3D doFirstPersonController(GameObject3D gameObject, KeySettings keySettings,
     ...
     * @return
     */
    public static final GameObject3D doThirdPersonController(GameObject3D source, GameObject3D destination,
	    KeySettings keySettings, float deltaPosXZSpeed, float deltaPosYSpeed, float deltaRotXYZSpeed) {
	final float dt = OmniKryptecEngine.instance().getDeltaTimef();
	deltaPosXZSpeed *= dt;
	deltaPosYSpeed *= dt;
	deltaRotXYZSpeed *= dt;
    ...
    /**
     * Moves the object the given distances, with ignoring the pitch and roll of the
     * object
     *
     ...
     */
    public static final void moveXZ(GameObject3D source, GameObject3D destination, float forward, float sideward,
	    float upward) {
	if (forward != 0) {
	    destination.getTransform().increasePosition(
		    (float) (forward * Math.sin(source.getTransform().getEulerAngelsXYZ(true).y)), 0,
		    (float) (-forward * Math.cos(source.getTransform().getEulerAngelsXYZ(true).y)));
	}
	if (sideward != 0) {
	    destination.getTransform().increasePosition(
		    (float) (sideward * Math.cos(source.getTransform().getEulerAngelsXYZ(true).y)), 0,
		    (float) (sideward * Math.sin(source.getTransform().getEulerAngelsXYZ(true).y)));
	}
	if (upward != 0) {
    ...
    /**
     * Moves the object the given distances, with using the pitch and roll of the
     * object
     *
     * @param forward  Positive = Forward, Negative = Backward
     * @param sideward Positive = Right, Negative = Left
     * @param upward   Positive = Up, Negative = Down
     */
    public static final void moveXYZ(GameObject3D source, GameObject3D destination, float forward, float sideward,
	    float upward) {
	if (forward != 0) {
	    destination.getTransform().increasePosition(
		    (float) (forward * Math.sin(source.getTransform().getEulerAngelsXYZ(true).y)),
		    (float) (-forward * Math.sin(source.getTransform().getEulerAngelsXYZ(true).x)),
		    (float) (-forward * Math.cos(source.getTransform().getEulerAngelsXYZ(true).y)
			    * Math.cos(source.getTransform().getEulerAngelsXYZ(true).x)));
	}
	if (sideward != 0) {
	    destination.getTransform().increasePosition(
		    (float) (sideward * Math.cos(source.getTransform().getEulerAngelsXYZ(true).y)
			    * Math.cos(source.getTransform().getEulerAngelsXYZ(true).y)),
		    (float) (-sideward * Math.sin(source.getTransform().getEulerAngelsXYZ(true).z)),
		    (float) (sideward * Math.sin(source.getTransform().getEulerAngelsXYZ(true).y)));
	}
	if (upward != 0) { // FIXME .y richtig?
	    destination.getTransform().increasePosition(
		    (float) (upward * Math.sin(source.getTransform().getEulerAngelsXYZ(true).z)),
		    (float) (upward * Math.cos(source.getTransform().getEulerAngelsXYZ(true).x)
			    * Math.cos(source.getTransform().getEulerAngelsXYZ(true).z)),
		    (float) (-upward * Math.sin(source.getTransform().getEulerAngelsXYZ(true).y)));
	}
    }

    public static final void rotateNormal(final GameObject3D destination, final float deltaRotX, final float deltaRotY,
	    final float deltaRotZ) {
	if (destination == null) {
	    return;
	}
    ...
    public static final void rotateNormal(final GameObject3D source, final GameObject3D destination,
	    final float deltaRotX, final float deltaRotY, final float deltaRotZ) {
	if (destination == null) {
	    return;
	}
	...
	if (source != null) {
	    ...
	    Logger.log("\nMaths.X == " + Maths.X + ", Maths.X Rotated == " + x_axis.toString() + "\nMaths.Y == "
		    + Maths.Y + ", Maths.Y Rotated == " + y_axis.toString() + "\nMaths.Z == " + Maths.Z
		    + ", Maths.Z Rotated == " + z_axis.toString());
	}
	// rotateNormal(destination, x_axis, y_axis, z_axis);
    }

    public static final void rotateNormal(final GameObject3D destination, final AxisAngle4f x_axis,
	    final AxisAngle4f y_axis, final AxisAngle4f z_axis) {
	if (destination == null) {
	    return;
	}
    ...
    public static final void turnXZ(GameObject3D source, GameObject3D destination, float deltaRotX, float deltaRotY,
	    float deltaRotZ) {
	final Vector3f rotation = source.getTransform().getEulerAngelsXYZ(true);
	if (deltaRotZ != 0) {
	    destination.getTransform().increaseRotation((float) (-deltaRotZ * Math.sin(rotation.y)), 0,
		    (float) (deltaRotZ * Math.cos(rotation.y)));
	}
	if (deltaRotX != 0) {
	    destination.getTransform().increaseRotation((float) (-deltaRotX * Math.cos(rotation.y)), 0,
		    (float) (deltaRotX * Math.sin(rotation.y)));
	}
	if (deltaRotY != 0) {
    ...
    public static final void turnXZY(GameObject3D source, GameObject3D destination, float deltaRotX, float deltaRotY,
	    float deltaRotZ) {
	final Vector3f rotation = source.getTransform().getEulerAngelsXYZ(true);
	if (deltaRotZ != 0) {
	    destination.getTransform().increaseRotation((float) (-deltaRotZ * Math.sin(rotation.y)),
		    (float) (deltaRotZ * Math.sin(rotation.x)),
		    (float) (deltaRotZ * Math.cos(rotation.y) * Math.cos(rotation.x)));
	}
	if (deltaRotX != 0) {
	    destination.getTransform().increaseRotation(
		    (float) (deltaRotX * Math.cos(rotation.y) * Math.cos(rotation.z)),
		    (float) (-deltaRotX * Math.sin(rotation.z)), (float) (deltaRotX * Math.sin(rotation.y)));
	}
	if (deltaRotY != 0) {
	    destination.getTransform().increaseRotation(0, deltaRotY, 0);
	}
    ...
    public static final void turnXYZ(GameObject3D source, GameObject3D destination, float deltaRotX, float deltaRotY,
	    float deltaRotZ) {
	final Vector3f rotation = source.getTransform().getEulerAngelsXYZ(true);
	if (deltaRotZ != 0) {
	    destination.getTransform().increaseRotation((float) (-deltaRotZ * Math.sin(rotation.y)),
	...
	if (deltaRotY != 0) {
	    destination.getTransform().increaseRotation((float) (deltaRotY * Math.sin(rotation.z)),
		    (float) (deltaRotY * Math.cos(rotation.x) * Math.cos(rotation.z)),
		    (float) (-deltaRotY * Math.sin(rotation.x)));
	}
    }

    public static final void turnNormal(GameObject3D source, GameObject3D destination, float deltaRotX, float deltaRotY,
	    float deltaRotZ) {
