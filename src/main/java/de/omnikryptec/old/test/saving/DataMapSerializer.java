...
public class DataMapSerializer {
	...
	for (Class<?> c : classesDataMaps.keySet()) {
		...
		for (DataMap d : data) {
		    try {
			final Object object = c.getMethod("newInstanceFromDataMap", d.getClass())
    ...
    public static final boolean serialize(String name, HashMap<Class<?>, ArrayList<DataMap>> classesDataMaps,
	    IDataMapSerializer dataMapSerializer, OutputStream outputStream) {
    ...
    public static final HashMap<Class<?>, ArrayList<DataMap>> deserializeThisToDataMap(InputStream inputStream,
    ...
    public static final HashMap<Class<?>, ArrayList<DataMapSerializable>> deserializeThisToDataMapSerializable(
	    InputStream inputStream, IDataMapSerializer dataMapSerializer) {
	return deserialize(deserializeThisToDataMap(inputStream, dataMapSerializer));
    }

    public final HashMap<Class<?>, ArrayList<DataMap>> deserializeToDataMap(InputStream inputStream,
	    IDataMapSerializer dataMapSerializer) {
	return (classesDataMaps = deserializeThisToDataMap(inputStream, dataMapSerializer));
    }

    public final HashMap<Class<?>, ArrayList<DataMapSerializable>> deserializeToDataMapSerializable(
	    InputStream inputStream, IDataMapSerializer dataMapSerializer) {
    ...
    public static final ArrayList<DataMap> getDataMaps(HashMap<Class<?>, ArrayList<DataMap>> classesDataMaps,
	    Class<?> type, boolean onlySame) {
	if (onlySame) {
	    return classesDataMaps.get(type);
	} else {
	    final ArrayList<DataMap> dataMaps = new ArrayList<>();
	    classesDataMaps.keySet().stream().forEach((c) -> {
		if (/* c.isAssignableFrom(type) || */type.isAssignableFrom(c)) { // FIXME Das auskommentierte kann
										 // eigentlich weg, aber noch nicht,
