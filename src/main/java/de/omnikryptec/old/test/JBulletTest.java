...
import de.omnikryptec.old.gameobject.FollowingCamera;
...
public class JBulletTest {
	...
	if (!physicsPause && physicsSpeed > 0) {
	    // TOD- dynamicsWorld.stepSimulation((1.0F / DisplayManager.instance().getFPS())
	    // * physicsSpeed);
	}
	...
	final Vector3f cameraPosition = ConverterUtil.convertVector3fFromLWJGL((camera instanceof FollowingCamera)
		? ((FollowingCamera) camera).getFollowedGameObject().getTransform().getPosition(true)
		: camera.getTransform().getPosition(true));
	...
	final Vector3f position = new Vector3f(relateTo.getTransform().getPosition().x, 35F,
		relateTo.getTransform().getPosition().z);
	...
	if (!(OmniKryptecEngine.instance().getCurrent3DScene().getCamera() instanceof FollowingCamera)
		&& OmniKryptecEngine.instance().getSettings().getKeySettings().getKey("mouseButtonLeft").isPressed()) {
	...
	try {
	    ...
	    DisplayManager.createDisplay("JBulletTest",
		    ...
		    new WindowInfo(4, 3, false, false, 1280, 720));
		...
		public void update() {
		    InputManager.doFirstPersonController(this,
			    OmniKryptecEngine.instance().getSettings().getKeySettings(), 3.0F, 1.5F, 15.0F, false);
		}
		...
		Entity followedEntity = new Entity(entityBuilder_brunnen.createTexturedModel()) {
		    ...
		    @Override
		    public void update() {
			InputManager.doThirdPersonController(
				OmniKryptecEngine.instance().getCurrent3DScene().getCamera(), this,
				OmniKryptecEngine.instance().getSettings().getKeySettings(), 10.0F, 5.0F, 40.0F); // Standard:
			// 1.5F,
			// 15.0F
			// (But
			// too
			...
			lustig.setCenterOfMassTransform(new Transform(new Matrix4f(
				new Quat4f(absrot.x, absrot.y, absrot.z, 1),
				ConverterUtil.convertVector3fFromLWJGL(getTransform().getPosition(true)), 1.0F)));
