...
import de.omnikryptec.old.gameobject.component.AudioListenerComponent3D;
import de.omnikryptec.old.gameobject.component.AudioSourceComponent3D;
import de.omnikryptec.old.gameobject.component.PhysicsComponent3D;
import de.omnikryptec.old.gameobject.terrain.Terrain;
import de.omnikryptec.old.gameobject.terrain.TerrainGenerator;
import de.omnikryptec.old.gameobject.terrain.TerrainTexturePack;
...
import de.omnikryptec.old.physics.JBulletPhysicsWorld;
import de.omnikryptec.old.physics.RigidBodyBuilder;
...
public class JBulletTest2 {
	...
	try {
	    ...
	    final GameSettings gameSettings = new GameSettings().setAnisotropicLevel(32).setMultisamples(32)
	    ...
	    keySettings.setKey(new KeyGroup("test_2", new Key("t_1", GLFW.GLFW_KEY_J, true),
		    new Key("t_2", GLFW.GLFW_KEY_K, true)));
	    ...
	    OmniKryptecEngine.instance().getSettings().getKeySettings().setKey("sprint", GLFW.GLFW_KEY_LEFT_CONTROL,
		    true);
	    ...
	    final SimpleTexture backgroundTexture = SimpleTexture
		    .newTextureb("/de/omnikryptec/old/gameobject/terrain/grassy2.png").create();
	    final SimpleTexture rTexture = SimpleTexture.newTextureb("/de/omnikryptec/old/gameobject/terrain/mud.png")
		    .create();
	    final SimpleTexture gTexture = SimpleTexture
		    .newTextureb("/de/omnikryptec/old/gameobject/terrain/grassFlowers.png").create();
	    final SimpleTexture bTexture = SimpleTexture.newTextureb("/de/omnikryptec/old/gameobject/terrain/path.png")
		    .create();
	    final SimpleTexture blendMap = SimpleTexture
		    .newTextureb("/de/omnikryptec/old/gameobject/terrain/blendMap.png").create();
	    try {
		heightMap = ImageIO.read(
			Terrain.class.getResourceAsStream("/de/omnikryptec/old/gameobject/terrain/heightmap.png"));
	    } catch (Exception ex) {
	    ...
	    entity_ball.getAdvancedModel().getMaterial().setTexture(Material.DIFFUSE,
		    entityBuilder_brunnen.getTexture());
	    ...
	    final StreamedSound streamedSound = StreamedSound.ofInputStream("Tobu_-_Infectious_[NCS_Release]", source,
		    JBulletTest2.class
			    .getResourceAsStream("/de/omnikryptec/old/audio/Tobu_-_Infectious_[NCS_Release].wav"));
	    ...
	    manageTerrains();
	    // entity_ball.addComponent(new PhysicsComponent(entity_ball,
	    // rigidBodyBuilder_ball));
	     ...
	     * input(); logic(); }, EventType.RENDER_FRAME_EVENT);
	     */
	...
	for (int x = 0; x < count; x++) {
		...
		final Terrain terrain = new Terrain(heightMap.getWidth(), x * heightMap.getWidth(),
			y * heightMap.getWidth(), "terrain", modelData, texturePack, blendMap);
	...
	for (Terrain terrain : terrains) {
	    ...
	    rigidBodyBuilder_terrain.setDefaultMotionState(new Vector3f(terrain.getTransform().getPosition().x / 2, 0,
		    terrain.getTransform().getPosition().z / 2), new Quat4f(0, 0, 0, 0));
	...
	((JBulletPhysicsWorld) OmniKryptecEngine.instance().getCurrent3DScene().getPhysicsWorld()).getWorld()
	...
	rigidBodyBuilder_ball.setDefaultMotionState(
		new Vector3f(camera.getTransform().getPosition().x, 20.0F, camera.getTransform().getPosition().z - 5),
		new Quat4f(0, 0, 0, 0));
	...
	rigidBodyBuilder_attractor.setDefaultMotionState(
		new Vector3f(camera.getTransform().getPosition().x, 10.0F, camera.getTransform().getPosition().z - 30),
		new Quat4f(0, 0, 0, 0));
	...
	final Vector3f attractorPosition = ConverterUtil
		.convertVector3fFromLWJGL(entity_attractor.getTransform().getPosition(true));
	...
	if (InputManager.isKeyboardKeyPressed(GLFW.GLFW_KEY_C)) {
	    ...
	    body.setCenterOfMassTransform(PhysicsUtil.createTransform(
		    ConverterUtil.convertVector3fFromLWJGL(camera.getTransform().getPosition(true)),
		    new Quat4f(0, 0, 0, 0)));
	...
	if (keySettings.getKey("mouseButtonLeft").isPressed()
		|| (InputManager.getCursorType() == CursorType.DISABLED)) {
	    if (InputManager.isKeyboardKeyPressed(GLFW.GLFW_KEY_L)) {
		InputManager.moveXZ(camera, camera, -deltaY / 15, -deltaX / 15, deltaD);
	    } else {
		OmniKryptecEngine.instance().getCurrent3DScene().getCamera().getTransform()
			.increaseRotation((deltaY / 5), -(deltaX / 5), 0);
	    }
	}
    }
	...
	temp.cross(bodyLocation,
		ConverterUtil.convertVector3fFromLWJGL(entity_ball.getTransform().getEulerAngelsXYZ()));
	temp.dot(force);
	Logger.log(temp);
	body.activate();
