...
public abstract class GameObject implements DataMapSerializable {
    ...
    /**
     * sets the updatetype of this GameObject. if its {@link UpdateType#SEMISTATIC}
     * the logic will be executed but the chunkposition etc will not be changed. if
     * its {@link UpdateType#STATIC} the logic will never be executed.
     *
     * @param t
     ...
     *
     * @param force if true all logic of this GameObject is executed, if neccessary
     *              or not (ignores the {@link UpdateType}).
     * @return this GameObject
     */
...
//        }
	...
	if (!c.supportsGameObject(this)) {
		...
		OmniKryptecEngine.instance().errorOccured(new OmniKryptecException(
			getClass().getSimpleName() + " does not support the component " + c.getClass().getSimpleName()),
			"Error while adding component to a GameObject");
	    } else {
		throw new OmniKryptecException(
			getClass().getSimpleName() + " does not support the component " + c.getClass().getSimpleName());
	    }
	}
	if (c.getPrio() < 0) {
     ...
     * {@link UpdateType#SEMISTATIC} pr {@link UpdateType#DYNAMIC} or if the logic
     * is forced.
     *
     * @see #doLogic(boolean)
     */
    ...
    /**
     * if true the gameobject is active and will be processed. if <code>false</code>
     * the logic will never be executed automatically.
     *
     * @param b
     */
...
//    }
	...
	for (GameObject gameObject : gameObjects) {
	    if ((!onlySame && c.isAssignableFrom(gameObject.getClass()) || (onlySame && gameObject.getClass() == c))
		    && (gameObject.getName() == null ? name == null : gameObject.getName().equals(name))) {
		return (T) gameObject;
	    }
	}
	return null;
...
//        data.put("transform", transform.toDataMap(new DataMap("transform")));
	...
	// return (gameObject != null ? gameObject : new
	// GameObject()).fromDataMap(data);
	return null;
    }
