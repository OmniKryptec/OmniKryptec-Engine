...
public class FollowingCamera extends Camera {
	...
	final float horizontalDistance = (float) (distanceFromGameObject * Math.cos(rot.x));
	final float verticalDistance = (float) (distanceFromGameObject * Math.sin(rot.x));
	...
	if (OmniKryptecEngine.instance().getDisplayManager().getSettings().getKeySettings().getKey("mouseButtonRight")
		.isPressed()) {
	    getTransform().getRotationSimple().rotateAxis((float) Math.toRadians(InputManager.getMouseDelta().y * 0.1F),
		    Maths.X);
	}
    }
