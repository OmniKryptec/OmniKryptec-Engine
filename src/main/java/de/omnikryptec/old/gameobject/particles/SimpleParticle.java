...
public class SimpleParticle extends Particle {
    ...
    public SimpleParticle(ParticleAtlas tex, Vector3f pos, Vector3f vel, Vector3f force, float lifeLength, float rot,
	    float startscale, float endscale, SimpleParticleSystem sys, RenderType type, float[] startcolor,
	    float endcolor[]) {
	super(pos, tex, type);
	setRotation(rot);
	this.velocity = vel;
	...
	timemultiplier = (mysystem == null ? 1 : mysystem.getTimeMultiplier())
		* OmniKryptecEngine.instance().getDeltaTimef();
