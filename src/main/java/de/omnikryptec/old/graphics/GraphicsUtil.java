...
public class GraphicsUtil {
	...
	if (needsUpdate(lastupdate, GameSettings.CHECKCHANGEFRAMES)) {
	    ...
	    rad_fol = OmniKryptecEngine.instance().getDisplayManager().getSettings()
		    .getFloat(GameSettings.RADIUS_FOLIAGE);
	    rad_med = OmniKryptecEngine.instance().getDisplayManager().getSettings()
		    .getFloat(GameSettings.RADIUS_MEDIUM);
	...
	if (type == RenderType.ALWAYS) {
	    ...
	    if (pos.lengthSquared() < cpos.lengthSquared() + rad_fol
		    && pos.lengthSquared() > cpos.lengthSquared() - rad_fol) {
		return true;
	    }
	} else if (type == RenderType.MEDIUM) {
	    cpos = c.getTransform().getPosition(true);
	    if (pos.lengthSquared() < cpos.lengthSquared() + rad_med
		    && pos.lengthSquared() > cpos.lengthSquared() - rad_med) {
		return true;
	    }
	} else if (type == RenderType.BIG) {
	    cpos = c.getTransform().getPosition(true);
	    if (pos.lengthSquared() < cpos.lengthSquared() + rad_big
		    && pos.lengthSquared() > cpos.lengthSquared() - rad_big) {
		return true;
	    }
	}
	return false;
	...
	return t == UpdateType.DYNAMIC
		? OmniKryptecEngine.instance().getDisplayManager().getFramecount() - frequenzy >= lastUpdate
		: false;
    }
